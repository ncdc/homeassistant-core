...
class AylaAPIHttpServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/local_lan/commands.json":
            ...
            data = (
                b'{"seq_no":'
                + str(device.seq_no).encode("utf-8")
                + b',"data":'
                + dataPendJsonStr.encode("utf-8")
                + b"}"
            )
            device.seq_no += 1
...
class Device:
    ...
    def ping(self, notify=0):
        try:
            logging.info(
                "Sending {} to {}\n".format(
                    "notify" if notify == 1 else "ping", api.devices[0].lan_ip
                )
            )
            r = requests.post(
                "http://" + api.devices[0].lan_ip + "/local_reg.json",
                json={
                    "local_reg": {
                        "uri": "/local_lan",
                        "notify": notify,
            ...
            logging.info(f"Request failed with exception {e!s}")
