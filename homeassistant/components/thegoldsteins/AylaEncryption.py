...
class AylaEncryption:
    ...
    def encryptAndSign(self, data):
        sign = bytes.fromhex(
            hmac.new(
                self.app_sign_key(), msg=data, digestmod=hashlib.sha256
            ).hexdigest()
        )
    ...
    def __init_aes_cipher(self):
        self.__encryptor = AES.new(
            self.app_crypto_key(), AES.MODE_CBC, self.app_iv_seed()
        )
        self.__decryptor = AES.new(self.app_dec_key(), AES.MODE_CBC, self.app_dec_iv())

    def __aylahmac(self, key, data):
        signature = bytes.fromhex(
            hmac.new(key, msg=data, digestmod=hashlib.sha256).hexdigest()
        )
        signature1 = bytes.fromhex(
            hmac.new(key, msg=signature + data, digestmod=hashlib.sha256).hexdigest()
        )
        return signature1
    ...
    def app_sign_key(self):
        ...
        data = (
            self.SRnd1.encode("utf-8")
            + self.SRnd2.encode("utf-8")
            + str(self.NTime1).encode("utf-8")
            + str(self.NTime2).encode("utf-8")
            + b"0"
        )
